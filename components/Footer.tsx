        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
          ...
          <nav className="flex flex-wrap items-center gap-x-6 gap-y-2">
            {[
              ...
              { href: "mailto:contacto@guiaholistica.com.ar", label: "Sumá tu perfil" },
              { href: "mailto:contacto@guiaholistica.com.ar", label: "Contacto" },
            ].map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="text-[13px] text-white/45 hover:text-warmblack transition-colors duration-300"
              >
                {link.label}
              </Link>
            ))}
