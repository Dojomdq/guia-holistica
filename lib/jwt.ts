...
async function getKey(secret: string): Promise<CryptoKey> {
  ...
  return crypto.subtle.importKey("raw", encoder.encode(secret), ALGORITHM, EXTRACTABLE, KEY_USAGES);
}
...
export async function signJWT(payload: JwtPayload, secret: string): Promise<string> {
  ...
  const body = base64url(JSON.stringify({ ...payload, iat: payload.iat ?? now, exp: payload.exp ?? now + 86400 }));
